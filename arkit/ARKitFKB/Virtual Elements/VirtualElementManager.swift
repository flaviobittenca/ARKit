...
class VirtualElementManager {
    ...
    var virtualObjects = [VirtualElement]()
    var lastUsedObject: VirtualElement?
    
    // MARK: - Resetting objects
    
    static let availableCategories: [VirtualCategoryDefinition] = {
        guard let jsonURL = Bundle.main.url(forResource: "VirtualElements", withExtension: "json")
            else { fatalError("missing expected VirtualElements.json in bundle") }
        do {
            let jsonData = try Data(contentsOf: jsonURL)
            return try JSONDecoder().decode([VirtualCategoryDefinition].self, from: jsonData)
        } catch {
            fatalError("can't load virtual objects JSON: \(error)")
        }
    }()
    
    static func availableObjects(for categoryIndex: Int) -> [VirtualElementDefinition] {
        return VirtualElementManager.availableCategories[categoryIndex].objects
    }
    ...
    func removeVirtualObject(at index: Int, for categoryIndex: Int) {
        let definition = VirtualElementManager.availableObjects(for: categoryIndex)[index]
    ...
    private func unloadVirtualObject(_ object: VirtualElement) {
    ...
    func loadVirtualObject(_ object: VirtualElement, to position: float3, cameraTransform: matrix_float4x4) {
    ...
    func translate(_ object: VirtualElement, in sceneView: ARSCNView, basedOn screenPos: CGPoint, instantly: Bool, infinitePlane: Bool) {
    ...
    func setPosition(for object: VirtualElement, position: float3, instantly: Bool, filterPosition: Bool, cameraTransform: matrix_float4x4) {
        if instantly {
    ...
    private func setNewVirtualObjectPosition(_ object: VirtualElement, to pos: float3, cameraTransform: matrix_float4x4) {
    ...
    private func updateVirtualObjectPosition(_ object: VirtualElement, to pos: float3, filterPosition: Bool, cameraTransform: matrix_float4x4) {
        let cameraWorldPos = cameraTransform.translation
        var cameraToPosition = pos - cameraWorldPos
    ...
    func transform(for object: VirtualElement, cameraTransform: matrix_float4x4) -> (distance: Float, rotation: Int, scale: Float) {
...
protocol VirtualObjectManagerDelegate: class {
    ...
    func virtualObjectManager(_ manager: VirtualElementManager, transformDidChangeFor object: VirtualElement)
    func virtualObjectManager(_ manager: VirtualElementManager, didMoveObjectOntoNearbyPlane object: VirtualElement)
